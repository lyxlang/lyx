...
(** [args] represents a list of argument given to a function *)
and args = Ast.arg located list [@@deriving show]

(** [signature] represents the type signature of a function *)
and signature = Ast.ann [@@deriving show]

(** [bind] corresponds to a binding. It has the form `(Var, Args, Signature,
    Expr, fv(Expr)) located` *)
and bind = (id * args * signature * expr located * ids) located
[@@deriving show]
...
let rec rebuild_let_binds (sccs : scc list) (ret : Ast.expr located) : Ast.expr
    =
  match sccs with
  | [] ->
      ret.value
  | AcyclicSCC {value= id, args, signature, body, _; loc} :: sccs' ->
      let name = Hashtbl.find id_map id in
      let bind = {id= name; args; signature; body} in
      ELet
        { bind= {value= bind; loc}
  ...
  | CyclicSCC binds :: sccs' ->
      let binds =
        List.map
          (fun {value= id, args, signature, body, _; loc} ->
            let name = Hashtbl.find id_map id in
            {value= {id= name; args; signature; body}; loc} )
          binds
      in
...
let rec rebuild_decl_binds (sccs : scc list) : Ast.decl list =
  let process_bind {value= id, args, signature, body, _; _} =
    let id =
      try
        let name = Hashtbl.find id_map id in
        L name.value
    ...
    in
    ({id= {value= id; loc= {start= 0; fin= 0}}; args; signature; body} : tlbind)
  in
  match sccs with
  | [] ->
      []
...
let rec analyze_arg scope arg =
  ...
  | AList lst | ATuple lst ->
      List.iter (fun arg -> analyze_arg scope arg.value) lst

(* process_expression : scope -> expr -> (fv, expr) *)
let rec process_expression (scope : scope) (expr : Ast.expr located) :
  ...
  | ELambda {args; body} ->
      ...
      in
      (* Collects the identifiers of lambda arguments (which are therefore
         bind) *)
      let ids = Hashtbl.fold (fun _ id acc -> id :: acc) varMap [] in
      (* Remove the lambda arguments from the free variables *)
      ...
      let binds : bind list =
        List.fold_left
          (fun acc bind ->
            ...
            let scope'' = Scope (new_map (), scope') in
            List.iter (fun arg -> analyze_arg scope'' arg.value) bind.value.args ;
    ...
    (fun decl ->
      ...
      | Decl {id; _} -> (
        ...
        | L name ->
            ...
            add_identifier root_scope name id_int )
      | _ ->
          () )
    program ;
  let binds : bind list =
    List.fold_left
      (fun acc decl ->
        ...
        | Decl {args; signature; body; _} ->
            let id_int = next_int () in
            (* (match id.value with | Wildcard -> () | L name -> Hashtbl.add
               id_map id_int { loc = id.loc; value = name }; add_identifier
               scope name id_int); *)
            let scope' = Scope (new_map (), root_scope) in
            List.iter (fun arg -> analyze_arg scope' arg.value) args ;
            { value=
                ( id_int
                , args
...
let analyze (program : Ast.program) =
  let sccs = process_program program in
  rebuild_decl_binds sccs
